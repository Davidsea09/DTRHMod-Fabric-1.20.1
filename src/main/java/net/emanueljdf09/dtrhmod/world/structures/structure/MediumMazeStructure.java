...
public class MediumMazeStructure extends Structure {
    public static final Codec<MediumMazeStructure> CODEC = createCodec(MediumMazeStructure::new);

    public MediumMazeStructure(Config config) {
        super(config);
    }
    ...
    public Optional<StructurePosition> getStructurePosition(Context context) {
        return Optional.of(new StructurePosition(context.chunkPos().getStartPos(), collector -> {
            ...
            MazeLayoutGenerator.generateAndPlaceMaze(collector, templateManager, originPos, context.random(), MazeLayoutGenerator.MazeSize.MEDIUM);
        }));
    }

    @Override
    public StructureType<?> getType() {
        return ModStructures.MEDIUM_MAZE_STRUCTURE_TYPE;
    }
}
